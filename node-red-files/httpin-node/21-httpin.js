...
module.exports = function(RED) {
    ...
    function HTTPIn(n) {
        ...
        if (RED.settings.httpNodeRoot !== false) {
            ...
            // Node callback. Everything starts here.
            this.callback = function(req,res) {
                ...
                // Fires when the client closes the connection with the server
				// Notify the flow that the connection was closed
                req.on('close', function () {
					console.log('HTTPIN: Connection closed, notifiying the flow');
					// TODO Have to add here more parameters in order (e.g.) to allow SWITCH to know the query parameters
					node.send({
    ...
    function HTTPOut(n) {
        ...
        this.streamResponse = (n.streamResponse == '1');
        
        this.on("input",function(msg) 
        {
            //console.log("HTTP Response input payload:");
            //console.log(msg.payload);
            //console.log('^ Payload ends over this sentence ^');
            
            var stream = this.streamResponse;
            if (msg.streamResponse !== undefined) stream = msg.streamResponse;
            console.log('Stream response is ' + stream);
            
            if (stream)
            {
                handleStreamResponse(msg, node, n);
            }
            else
            {
                handleRegularResponse(msg, node, n);
            }
            console.log("Done: HTTPOut input management finished");
        });
    }
    
    function handleRegularResponse(msg, node, n)
    {
        console.log('In handleRegularResponse()');
        if (msg.res) {
    ...
    function handleStreamResponse (msg, node, n)
    {            
        console.log('In handleStreamResponse()');;
        // Ensure a proper msg
        if (msg.res) 
        {
            // Headers can be set just once, at the first response
            if (!msg.res._res.headersSent) 
            {
                // Headers not sent, sent them
                var headers = RED.util.cloneMessage(node.headers);
                if (msg.headers) {
                    if (msg.headers.hasOwnProperty('x-node-red-request-node')) {
                        var headerHash = msg.headers['x-node-red-request-node'];
            ...
            // Write the response to the stream                
                // AITOR _res es un objeto serverResponse de Express https://expressjs.com/en/api.html#res.send, 
                // que a su vez hereda de serverResponse de node: https://nodejs.org/api/http.html#http_class_http_serverresponse
                // Stream en Express: https://stackoverflow.com/questions/38788721/how-do-i-stream-response-in-express
                // send() es para peticiones no streaming y fija el content-length siempre, si no viene dado
